...
- Add a new plot to an existing shiny app
...
downloadthis::download_link(
  link = 'https://github.com/ccgh-idd/shiny-intro/data/shiny-app-init.zip',
...
Every Shiny app has **three essential components**:
...
3. **App call**: launches the application
...
Layout functions control how your app is structured visually. The most common pattern is:

- **`fluidPage()`**: Creates a responsive page that adjusts to different screen sizes
- **`sidebarLayout()`**: Divides the page into two panels - a sidebar and a main panel
  - **`sidebarPanel()`**: Typically contains input controls (filters, sliders, etc.)
  - **`mainPanel()`**: Displays outputs (plots, tables, text)
- **`titlePanel()`**: Adds a title at the top of the page

Other useful layout functions include `fluidRow()` and `column()` for creating custom grid layouts, and `tabsetPanel()` for organizing content into tabs.
...
*user inputs* → *data manipulation* → *outputs*
...
*User changes input* → *Reactive expression updates* → *Output re-renders*
...
In Shiny, you create tabs using:

- **`tabsetPanel()`**: Creates the container for tabs
- **`tabPanel()`**: Defines each individual tab with a title and content
...
mainPanel(
  tabsetPanel(
    tabPanel("Tab 1 Title", 
             plotOutput("plot1")),
    tabPanel("Tab 2 Title", 
             tableOutput("table1"))
  )
)
```

Each `tabPanel()` can contain multiple outputs (plots, tables, text) just like the main panel can.

::: {.write}
**Step 1: Restructure the UI with tabsets**

Modify your `mainPanel()` in the UI to use a tabset structure:

1. Below the epicurve, add a new section with a `tabsetPanel()` for the Person Analysis
...
Ok now your `mainPanel()` should look like:
...
**R packages**
- [epishiny](https://epicentre-msf.github.io/epishiny/) - provides simple functions that produce engaging, feature-rich interactive visualisations and dashboards from epidemiological data using R’s shiny web-framework. (developped by [Epicentre Data Science Team](https://epicentre-msf.github.io/gallery/))
- [shinyWidgets](https://github.com/dreamRs/shinyWidgets) - gives you many many more widgets that can be used in your app.epishi
- [highcharter](https://jkunst.com/highcharter/) - R wrapper for the Highcharts (javascript) library for interactive visualisations