---
title: Matching names with {nmatch}
description: Learn an efficient algorithm for complex name matching using the package {nmatch}
date: last-modified
format:
  html:
...
Objectives
----------------------------------------------------------------------------------------------------
- Explore the nmatch package and the concept of fuzzy matching
- Implement fuzzy name matching at scale
- Design and apply tiered matching criteria
- Validate and reconcile matches through manual review

Ressources
----------------------------------------------------------------------------------------------------
During this session we will be using some R packages developped by Epicentre [Data Science team](https://epicentre-msf.github.io/gallery/), so please make sure it is downloaded.
...
You can also check other of our packages [here](https://epicentre-msf.r-universe.dev/packages) ! 

Please also make sure that these packages are installed and loaded: 

```{r}
library(janitor) #data wrangling and cleaning
...
For this session we will be using data simulated from a measles outbreak in Southern Chad. These data are shared as a `.rds` file that contains a list: 

| Dataset | Type | Description | Contents |
|---------|------|-------------|-----------------------|
| `msf_linelist` | Dataframe | Epidemiological linelist | • 5,230 probable cases of Measles<br>• Key epidemiological dates<br>• Demographic information |
| `lab_data` | Dataframe | Laboratory database | • List of all laboratory test performed<br> |

```{r}
#| echo: false
#| eval: true
# msf_linelist <- episimdata::moissala_linelist_EN |> as_tibble()
# lab_data <- episimdata::moissala_laboratory_clean_EN |>select(-id) |> as_tibble()

# data_ls <- list(
#   "msf_linelist" = msf_linelist, 
#   "lab_data" = lab_data
# )

# saveRDS(data_ls, here::here("data", "clean", "moissala_data.rds"))
